...
public class InstanceStats extends InitializerOnce
{
	public void initialize()
	{
		Index index = new Index();
		index.addMapping(new String[] { "stats" });
		index.description = "Manages instance stats";
		Handler.addHandler("/busit/instance/", index);
		
		initializeInsert(index);
		initializeSelect(index);
		
		Self.selfize("/busit/instance/stats/select");
	}
	...
	private void initializeInsert(Index index)
	{
		Action insert = new Action()
		{
			public Object execute() throws Exception
			{
				...
				Map<String, String> check = Database.getInstance().selectOne("SELECT stat_id FROM instance_stats WHERE stat_instance = '" + Security.escape(instance) + "' AND stat_timestamp = " + Security.escape(timestamp));
				if( check != null && check.get("stat_id") != null ) 
				{
					Database.getInstance().update("UPDATE instance_stats SET stat_messages = '" + Security.escape(messages) + "', stat_size = '" + Security.escape(size) + "', stat_credits_user = '" + Security.escape(credits_user) + "', stat_credits_developer = '" + Security.escape(credits_developer) + "', stat_credits_busit = '" + Security.escape(credits_busit) + "' WHERE stat_instance = '" + Security.escape(instance) + "' AND stat_timestamp = " + Security.escape(timestamp));
				}
				else
				{
					Database.getInstance().insert("INSERT INTO instance_stats (stat_instance, stat_timestamp, stat_messages, stat_size, stat_credits_user, stat_credits_developer, stat_credits_busit) " + 
					"VALUES ('" + Security.escape(instance) + "', '" + Security.escape(timestamp) + "', '" + Security.escape(messages) + "', '" + Security.escape(size) + "', '" + Security.escape(credits_user) + "', '" + Security.escape(credits_developer) + "', '" + Security.escape(credits_busit) + "')");
				}
	...
	private void initializeSelect(Index index)
	{
		Action select = new DelegateAction()
		{
			public Object execute() throws Exception
			{
				...
				String where = "";
				if( instance != null )
					where += " AND stat_instance = " + Security.escape(instance);
				if( user != null )
				...
				if( group != null )
				{
					return Database.getInstance().select("SELECT SUM(stat_messages) as messages, SUM(stat_size) as size, SUM(stat_credits_user) as credits_user, SUM(stat_credits_developer) as credits_developer, SUM(stat_credits_busit) as credits_busit, " + group + "(FROM_UNIXTIME(stat_timestamp)) as " + group + " " + 
						"FROM instance_stats " + 
						"LEFT JOIN instances ON(instance_id = stat_instance) " +
		...
		select.description = "Select instance stats.";
		...
		instance.mustMatch = PatternBuilder.getRegex(PatternBuilder.NUMBER);
		instance.description = "The instance id";
		instance.addAlias(new String[]{ "instance", "id", "instance_id", "iid" });
