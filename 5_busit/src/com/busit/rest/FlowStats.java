...
public class FlowStats extends InitializerOnce
{
	public void initialize()
	{
		Index index = new Index();
		index.addMapping(new String[] { "stats" });
		index.description = "Manages flow stats";
		Handler.addHandler("/busit/flow/", index);
		
		initializeInsert(index);
		initializeSelect(index);
		
		Self.selfize("/busit/flow/stats/select");
	}
	...
	private void initializeInsert(Index index)
	{
		Action insert = new Action()
		{
			public Object execute() throws Exception
			{
				...
				Map<String, String> check = Database.getInstance().selectOne("SELECT stat_id FROM flow_stats WHERE stat_flow = '" + Security.escape(flow) + "' AND stat_timestamp = " + Security.escape(timestamp));
				if( check != null && check.get("stat_id") != null ) 
				{
					Database.getInstance().update("UPDATE flow_stats SET stat_messages = '" + Security.escape(messages) + "', stat_size = '" + Security.escape(size) + "', stat_credits_user = '" + Security.escape(credits_user) + "', stat_credits_developer = '" + Security.escape(credits_developer) + "', stat_credits_busit = '" + Security.escape(credits_busit) + "' WHERE stat_flow = '" + Security.escape(flow) + "' AND stat_timestamp = " + Security.escape(timestamp));
				}
				else
				{
					Database.getInstance().insert("INSERT INTO flow_stats (stat_flow, stat_timestamp, stat_messages, stat_size, stat_credits_user, stat_credits_developer, stat_credits_busit) " + 
					"VALUES ('" + Security.escape(flow) + "', '" + Security.escape(timestamp) + "', '" + Security.escape(messages) + "', '" + Security.escape(size) + "', '" + Security.escape(credits_user) + "', '" + Security.escape(credits_developer) + "', '" + Security.escape(credits_busit) + "')");
				}
		...
		flow.addAlias(new String[]{ "flow", "id", "flow_id" });
		insert.addParameter(flow);	
	...
	private void initializeSelect(Index index)
	{
		Action select = new DelegateAction()
		{
			public Object execute() throws Exception
			{
				...
				String where = "";
				if( flow != null )
					where += " AND stat_flow = " + Security.escape(flow);
				if( user != null )
				...
				if( group != null )
				{
					return Database.getInstance().select("SELECT SUM(stat_messages) as messages, SUM(stat_size) as size, SUM(stat_credits_user) as credits_user, SUM(stat_credits_developer) as credits_developer, SUM(stat_credits_busit) as credits_busit, " + group + "(FROM_UNIXTIME(stat_timestamp)) as " + group + " " + 
						"FROM flow_stats " + 
						"LEFT JOIN spaces ON(space_id = stat_flow) " +
		...
		select.description = "Select flow stats.";
		select.returnDescription = "The matching logs [{'identifier', 'flow'},...]";
		select.addGrant(new String[] { "access", "instance_select" });
		
		Parameter flow = new Parameter();
		flow.isOptional = true;
		flow.minLength = 1;
		flow.maxLength = 20;
		flow.mustMatch = PatternBuilder.getRegex(PatternBuilder.NUMBER);
		flow.description = "The flow id";
		flow.addAlias(new String[]{ "flow", "id", "flow_id", "fid" });
		select.addParameter(flow);
